...
@Api(tags="自动化测试用例管理")
@RestController
@RequestMapping("/task/autoRunningCaseManage")
@Slf4j
public class AuToRunningCaseManageController extends JeecgController<RunningCase, IAuToRunningCaseManageService> {
    @Autowired
    private IAuToRunningCaseManageService auToRunningCaseManageService;
    ...
    public Result<?> queryPageList(@RequestParam(name="testTaskId", required = false) String testTaskId,
        ...
        pageList = auToRunningCaseManageService.queryPageList(pageList,testTaskId,projectId,testName,testCode,testPerson,testDate);
    ...
    public Result<?> add(@RequestBody RunningCase runningCase) {
        ...
        auToRunningCaseManageService.save(runningCase);
        String copy="copy";
    ...
    public Result<?> edit(@RequestBody RunningCase runningCase) {
        auToRunningCaseManageService.updateById(runningCase);
        //修改添加历史记录
    ...
    public Result<?> delete(@RequestParam(name="id",required=true) String id) {
        RunningCase runningCase=auToRunningCaseManageService.getById(id);
    ...
    public Result<?> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
        List<RunningCase> runningCaseList=auToRunningCaseManageService.listByIds(Arrays.asList(ids.split(",")));
        for (RunningCase runningCase:runningCaseList){
            runningCase.setDelFlag(1);
            runningCase.setUpdateTime(new Date());
            insertHistory(runningCase,"2",runningCase.getId());
        }
        auToRunningCaseManageService.updateBatchById(runningCaseList);
    ...
    public Result<?> queryCopyDataById(@RequestParam(name="id",required=true) String id) {
        List<RunningCase> caseList  = auToRunningCaseManageService.getCopyDataById(id);
        if(caseList==null) {
            return Result.error("未找到对应数据");
        }
    ...
    public Result<?> queryTestCase(@RequestParam(name="id",required=true) String id) {
        ...
        //根据任务id,查询被测对象体系id
        String templateId=auToRunningCaseManageService.getTestTemplateById(id);
        if(!templateId.isEmpty()) {
            //查询树形所需数据
            list  = auToRunningCaseManageService.queryTreeList(templateId);
    ...
    public Result<?> getProjectNameByTaskId(@RequestParam(name="taskId",required=true) String taskId) {
        RunningProject runningProject = auToRunningCaseManageService.getProjectNameByTaskId(taskId);
    ...
    public Result<?> getPersonDataByTaskId(@RequestParam(name="taskId",required=true) String taskId) {
        //根据任务ID查询项目ID
        RunningTask projectIds = auToRunningCaseManageService.getProjectIdByTaskId(taskId);
    ...
    public ModelAndView exportXls(HttpServletRequest request, RunningCaseManageVO runningCaseManageVO) {
        ...
        pageList = auToRunningCaseManageService.getRunningCaseManageData1();
